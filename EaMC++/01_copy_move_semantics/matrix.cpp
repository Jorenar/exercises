...
Matrix::Matrix() :
    N(0), M(0), data(new double[1])
{
    data[0] = 0;
}

Matrix::Matrix(size_t N_, size_t M_) :
    N(N_), M(M_), data(new double[N*M] {0})
{
...
{
    data = new double[N*M];
    std::copy(obj.data, obj.data+(obj.N*obj.M), data);
...
{
    Matrix temp(N, M);
    std::transform(data, data+N*M, temp.data, std::negate<double>());
    return temp;
}
