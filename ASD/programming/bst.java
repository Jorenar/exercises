...
class Tree {
    ...
    private int[] isAVL(Node t) { // recursive, each node visited only once
        if (t == null) {
            return new int[] {1, -1};
        }
    ...
    public Stack<Int> findPath(int x) {
        Stack<Int> X = new Stack<Int>();
        Node t = root;
        while (t != null && t.info != x) { // one loop - linear complexity
            X.push(new Int(t.info));
            t = x < t.info ? t.left : t.right;
        }
        return X;
    }
    ...
    public int distance(int x, int y) {
        ...
        Stack<Int> X = findPath(x).push(new Int(x)).reverse();
        Stack<Int> Y = findPath(y).push(new Int(y)).reverse();

        while (!(X.empty() || Y.empty()) && X.peek().val == Y.peek().val) {
            X.pop();
            Y.pop();
        }
            ...
            for (int y = 0; y < Y; ++y) {
                ...
                switch (cmd[0]) {
                    ...
                    case "SCAN":
                        ...
                        break;
                    case "AVL":
                        System.out.println(cmd[0] + ": " + (bst.isAVL() ? "yes" : "no"));
                        break;
                    case "DISTANCE":
                        int u = Integer.parseInt(cmd[1]);
                        int v = Integer.parseInt(cmd[2]);
                        System.out.println(cmd[0] + ": " + bst.distance(u, v));
                        break;
                }
            }
            System.out.println();
